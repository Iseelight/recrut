...
import { FaceDetectionData, SecurityAlert } from '../../types';
...
export function FloatingVideoMonitor({ 
  ...
  const generateSecurityAlert = useCallback((type: SecurityAlert['type'], message: string, severity: SecurityAlert['severity']) => {
  ...
  const detectFaces = useCallback(() => {
    ...
    try {
      ...
      if (!baselineRef.current.established && pixelCount > 0) {
        if (avgBrightness > 30) { // Only establish baseline if there's reasonable lighting
          ...
          <div className="relative mt-6">
            ...
            {!faceDetectionData.faceDetected && (
              <div className="absolute top-6 left-0 right-0 bg-red-500/90 text-white text-center py-1 animate-pulse">
                ...
                {countdownSeconds > 0 && (
                  <div className="text-xs font-bold">{countdownSeconds}s remaining</div>
                )}
              </div>
            )}
            ...
            <div className="absolute bottom-2 left-2 flex space-x-1">
              ...
              {isVideoEnabled && (
                ...
                `}>
                  {faceDetectionData.faceDetected ? <Eye className="w-3 h-3 mr-1" /> : <EyeOff className="w-3 h-3 mr-1" />}
            ...
            {isVideoEnabled && (
              <div className="absolute top-2 right-2 flex flex-col space-y-1">
                <div className={`
                  ...
                  ${faceDetectionData.confidence > 0.7 ? 'bg-green-500/90 text-white' :
                    faceDetectionData.confidence > 0.4 ? 'bg-yellow-500/90 text-black' :
                    'bg-red-500/90 text-white'
                  }
                `}>
      ...
      {isMinimized && (
        <div className="w-full h-full flex items-center justify-center">
          ...
          `}>
            ...
            {isVideoEnabled ? (
              faceDetectionData.faceDetected ? <Eye className="w-4 h-4" /> : <User className="w-4 h-4" />
            ) : (
